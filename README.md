...
Ferroflow reads `config.yml` on startup. Use `config.example.yml` as a starting point. Heartbeat retries use `heartbeat.period` as the base interval and back off per node when heartbeat responses are missing.
