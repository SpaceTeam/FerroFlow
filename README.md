...
**Rust schema**
Since diesel-cli cannot generate a schema for tables without a primary key, we maintain a manually written schema for the telemetry_logs table in `src/db/timescale_schema.rs`. For all other (future) tables, the schema is (would be) automatically generated by diesel-cli and located in `src/db/schema.rs`.